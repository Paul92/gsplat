...
###### RaDe-GS INRIA Wrapper ######

def rasterization_rade_inria_wrapper(
    means: Tensor,  # [N, 3]
    quats: Tensor,  # [N, 4]
    scales: Tensor,  # [N, 3]
    opacities: Tensor,  # [N]
    ...
    **kwargs,
    ...
    for cid in range(C):
        ...
        for i in range(0, channels, 3):
            ...
            _render_colors_, radii, coord, mcoord, depth, mdepth, alpha, normal = rasterizer_output
            if _colors.shape[-1] < 3:
                _render_colors_ = _render_colors_[:, :, : _colors.shape[-1]]
            render_colors_.append(_render_colors_)
        render_colors_ = torch.cat(render_colors_, dim=-1)
    ...
    # additional maps
    render_depth_expected = depth.unsqueeze(-1)
    render_alphas = alpha.unsqueeze(-1)
    render_normal = normal
    render_depth_median = mdepth.unsqueeze(-1)
