...
def get_smallest_axis(means, quats, scales, viewmat, return_idx=False):
...
def get_normal(means, quats, scales, viewmat):
...
def compute_all_map(means, quats, scales, viewmat):
    global_normal = get_normal(means, quats, scales, viewmat)
...
def compute_depths(viewmats, means):
...
def normalize_normal_map(normal_map: torch.Tensor) -> torch.Tensor:
    """
    Normalizes a normal map so that each normal vector has a unit length.

    Args:
        normal_map (torch.Tensor): The normal map with shape [H, W, 3].

    Returns:
        torch.Tensor: The normalized normal map with shape [H, W, 3].
    """
    # Compute the magnitude of each normal vector
    magnitude = torch.norm(normal_map, dim=-1, keepdim=True)
    ...
    normalized_normal_map = normal_map / magnitude
...
def render_normal(K, viewmatrix, depth, offset=None, normal=None, scale=1):
    # depth: (H, W), bg_color: (3), alpha: (H, W)
    # normal_ref: (3, H, W)
    intrinsic_matrix = K
    extrinsic_matrix = viewmatrix.transpose(0,1).contiguous()
...
def rasterization_pgsr(
    ...
    # viewmats = torch.zeros_like(viewmats).to(device)
    # viewmats[:, :3, :3] = torch.eye(3).to(device)
    # # viewmats[:, :3, 3] = torch.tensor([0, 0, 0]).to(device)
    ...
    # print("rank", world_rank, "Before rasterize_to_pixels")
    if colors.shape[-1] > channel_chunk:
        raise NotImplementedError("Channel chunk is not supported yet.")
        # slice into chunks
        n_chunks = (colors.shape[-1] + channel_chunk - 1) // channel_chunk
        render_colors, render_alphas = [], []
        for i in range(n_chunks):
        ...
        all_maps = compute_all_map(means, quats, scales, viewmats[0])
        # all_maps[..., 4] = depths
        ...
        render_colors, render_alphas, render_observe, render_all_map, render_depths = rasterize_to_pixels_pgsr(*input)

        meta["render_observe"] = render_observe
        meta["render_all_map"] = render_all_map
        meta["render_depths"] = render_depths
        meta["render_normals"] = render_all_map[..., :3]

        depth_normal = render_normal(Ks[0], viewmats[0], render_depths.squeeze()).unsqueeze(0)
        depth_normal = depth_normal * (render_alphas).detach()
        meta.update({"depth_normals": depth_normal})

        # meta["render_normals"] = normalize_normal_map(meta["render_normals"])
        # meta["depth_normals"] = normalize_normal_map(meta["depth_nzormals"])
...
def rasterization_inria_pgsr_wrapper(
    means: Tensor,  # [N, 3]
    quats: Tensor,  # [N, 4]
    scales: Tensor,  # [N, 3]
    opacities: Tensor,  # [N]
    ...
    **kwargs,
    ...
    for cid in range(C):
        ...
        means2D = torch.zeros_like(means, requires_grad=True, device=device)
        means2D_abs = torch.zeros_like(means, requires_grad=True, device=device)
        ...
        for i in range(0, channels, 3):
            ...
            if _colors.shape[-1] < 3:
                ...
                _colors = torch.cat([_colors, pad], dim=-1)
            _render_colors_, radii, out_observe, out_all_map, out_plane_depth = rasterizer(
                means3D=means,
                means2D=means2D,
                ...
                rotations=quats,
                cov3D_precomp=None,
                all_map=compute_all_map(viewmats[cid]),
            )
            if _colors.shape[-1] < 3:
                _render_colors_ = _render_colors_[:, :, : _colors.shape[-1]]
            render_colors_.append(_render_colors_)
        ...
        render_colors.append(render_colors_)
    render_colors = torch.stack(render_colors, dim=0)
    return render_colors, None, {"observe": out_observe, "all_map": out_all_map, "depths": out_plane_depth}